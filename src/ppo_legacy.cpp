...
class CPointSet
{
    ...
    DT dtFixed;                                                                 // Delaunay triangulation with fixed tile in the middle, surrounded by 8 copies of the adjustable tile.
    double maxShift;
    void updateFaceInfo() {
    ...
    struct TSite {                                                              // This is to handle the Delaunay triangulation.
        ...
        VH vhFixed[8];                                                          // Handles all but the center replicas to allow BN on two Tiles
    ...
    Point setSiteOnlyCenterReplica(int index, Point p);
    ...
    Vector conflictFixed(int index);
    Vector coverageFixed(int index);
    Vector capacitySerialFixed(int index);
...
public:
    ...
    CPointSet(int nPoints, double* inputPoints, double aspectRatio);
    CPointSet(int nPoints, double* inputPoints, double* inputPointsFixedTile, double aspectRatio);
...
CPointSet::CPointSet(int nPoints, double* inputPoints, double* inputPointsFixedTile, double aspectRatio) : CPointSet(nPoints, aspectRatio) // todo: 
{
    for (auto i = 0; i < n; ++i)
    {
        Point p(inputPoints[i], inputPoints[i + n]);
...
Point CPointSet::setSite(int index, Point p) {                                  // Set location of the indexed point (in t-domain) and insert it in triangulation
	...
	for (int i = 0; i < 8; i++)
	{
        sites[index].vhFixed[i] = dtFixed.insert(replica(p, i + 1));     // insert replica in triangulation and keep handle to it avoid replica 0 which is the center tile
        sites[index].vhFixed[i]->info().id = index;                              // Point the DT point back to map entry
	}
    return p;
}

Point CPointSet::setSiteOnlyCenterReplica(int index, Point p)
{
    p = mainReplica(p);
    sites[index].p = p;
    sites[index].isFixed = true;

    sites[index].vhFixed[0] = dtFixed.insert(p);
    sites[index].vhFixed[0]->info().id = index;
    return p;
}
...
void CPointSet::moveSite(int index, Point p) {                                  // Adjust location of indexed point (in t-domain) and update in triangulation
    ...
    for (auto i = 0; i < 8; i++) {
        if (sites[index].vhFixed[i] != NULL)
            sites[index].vhFixed[i] = dt.move(sites[index].vhFixed[i], replica(p, i+1));
    }
...
Vector CPointSet::capacitySerialFixed(int i) // todo:
{
    if (sites[i].isFixed) return { 0,0 };
    double d[20];    // Distance to neighbor (= 2 x distance to Voronoi edge)
    double el[20];   // length of the voronoi edges
    double area = 0; // Area of Voronoi cell
    FC fc2 = dtFixed.incident_faces(sites[i].vh[0]), fc1(fc2++);                     // fc1 and fc2 are two consecutive (ccw) faces incident to current vertex
    VC vc = dtFixed.incident_vertices(sites[i].vh[0], fc2), done(vc);                // The vertex sharing fc1 and fc2 with v[i].vh
    int m = 0;                                                                  // Number of neighbors
    Vector dir[20];                                                             // Direction vectors to neighbors
    int id[20];                                                                 // Id's of neighbors. We can't use the circulator for updating
    do {
        Point c1 = dtFixed.circumcenter(fc1), c2 = dtFixed.circumcenter(fc2);             // Circumcenters of faces are endpoints of Voronoi cell edge
        area += c1.x() * c2.y() - c1.y() * c2.x();                              // Accumulate areas (rhs is cross product)
        el[m] = sqrt((c2 - c1).squared_length());                               // Length of Voronoi edge
        dir[m] = (vc->point() - sites[i].p);
        d[m] = sqrt(dir[m].squared_length());
...
// test conflict resolution by pushing neighbors
Vector CPointSet::conflictFixed(int index) // todo: 
{
    if (sites[index].isFixed) return { 0,0 };
    double dmin = rel_dmin * dhex;
    Point& p = sites[index].p;
    bool conflict[30];
    Vector shift[30];
    int id[30];
    int m = 0;
    VC vc = dtFixed.incident_vertices(sites[index].vh[0]), done(vc);
    do {
        Vector edge = vc->point() - p;
        double l = VL(edge);
        if (l < dmin) {
    ...
    for (int i = 0; i < m; i++) {
        if (!sites[id[i]].isFixed && conflict[i])
            moveSite(id[i], shift[i]);
    }
    return Vector(0, 0);
}
...
// A coverage routine which pulls neighbors
Vector CPointSet::coverageFixed(int index)
{
    if (sites[index].isFixed) return { 0,0 };
    double rc = rel_rc * dhex;
    double dmin = rel_dmin * dhex;
    ...
    Vector edge[30];
    int m = 0;
    FC fc = dtFixed.incident_faces(sites[index].vh[0]), done(fc);
    VC vc = dtFixed.incident_vertices(sites[index].vh[0], fc);
    do {
        vc++;
        edge[m] = sites[index].p - vc->point();
        id[m] = vc->info().id;
        if (triangleType(fc) <= 0) {
            Point c = dtFixed.circumcenter(fc);
            double l = VL(c - sites[index].p);
            scale[m] = rc / l;
        }
        else scale[m] = 2; // > 1
...
double CPointSet::PPO_serial(std::string seq) {
    ...
    for (int i = 0; i < n; i++) {                                               // Iterate through numbers up to maximum index
        ...
        for (int k = 0; k < seq.length(); k++) {
            switch (seq[k]) {
            case '0': std::cout << "cov" << std::endl; coverage(index); std::cout << " done" << std::endl; break;
            case '1': std::cout << "conf" << std::endl; conflict(index); std::cout << "done" << std::endl; break;
            case '2': std::cout << "cap" << std::endl; capacitySerial(index); std::cout << "done" << std::endl; break;
            case '3': std::cout << "covF" << std::endl; coverageFixed(index); std::cout << "done" << std::endl; break;
            case '4': std::cout << "confF" << std::endl; conflictFixed(index); std::cout << "done" << std::endl; break;
            case '5': std::cout << "capF" << std::endl; capacitySerialFixed(index); std::cout << "done" << std::endl; break;
            }
        }
    }
    delete[] order;
...
void optimizePattern(double dMin, double rC, double areaDeltaMax, unsigned int nPoints, double* inMatrix, double* inMatrixFixedTile, double* outMatrix, double aspectRatio)
{
    ...
    CPointSet ps(nPoints, inMatrix, inMatrixFixedTile, aspectRatio);
    optimizePattern(dMin, rC, areaDeltaMax, std::move(ps), outMatrix, aspectRatio);
}